...
public class ColorPickerDialog extends Dialog {
	...
	public ColorPickerDialog(Context context, String title,
			OnColorChangedListener listener,int position) {
		this(context, Color.BLACK, title, listener,position);
	}
	/**
	 * 
	 * @param context
		...
		public boolean onTouchEvent(MotionEvent event) {
			...
			case MotionEvent.ACTION_MOVE:
				...
				if (downInCircle && inCircle) {// down按在渐变色环内, 且move也在渐变色环内
					...
					int circleR = Color.red(interpCircleColor(mCircleColors, unit));
					int circleG = Color.green(interpCircleColor(mCircleColors, unit));
					int circleB = Color.blue(interpCircleColor(mCircleColors, unit));
					MqttV3Service.publishMsg("RGB:"+circleR+","+circleG+","+circleB+";", 1,1);
					mCenterPaint
					...
					int circleR = Color.red(interpRectColor(mRectColors, x));
					int circleG = Color.green(interpRectColor(mRectColors, x));
					int circleB = Color.blue(interpRectColor(mRectColors, x));
					
					MqttV3Service.publishMsg("RGB:"+circleR+","+circleG+","+circleB+";", 1,position);
				}
				if (debug)
			...
			case MotionEvent.ACTION_UP:
				if (highlightCenter && inCenter) {// 点击在中心圆, 且当前启动在中心圆
					if (mListener != null) {
						mListener.colorChanged(mCenterPaint.getColor());
						System.out.println("mCenterPaint "+mCenterPaint.getColor());
